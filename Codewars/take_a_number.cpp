...
The number 89 89 is the first integer with more than one digit that fulfills the property partially introduced in
...
using namespace std;
vector<unsigned int> dat = { 1, 2, 3, 4, 5, 6, 7, 8, 9, 89, 135, 175, 518, 598, 1306, 1676, 2427, 2646798};

std::vector<unsigned int> sumDigPow(unsigned int a, unsigned int b) {
    vector<unsigned int> res={};
    int n = dat.size();
    for (int j = 0; j<n;j++){
       if ((dat[j]>=a) && (dat[j]<=b)) res.push_back(dat[j]);
    }
    return res;
}
